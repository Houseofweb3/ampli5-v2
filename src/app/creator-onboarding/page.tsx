...
export default function CreatorOnboardingForm() {
    ...
    const [numberPickupCountry, setNumberPickupCountry] = useState<string>('us');
    ...
    useEffect(() => {
        ...
        const selectedInventoryItems = Object.entries(inventoryItems).filter(([_, item]) => item.selected);
        if (selectedInventoryItems.length > 0 && selectedInventoryItems.every(([_, item]) => item.rate && item.rate.trim() !== '')) completed.add(4);
        ...
        switch (step) {
            ...
            case 4:
                ...
                if (selectedInventoryItems.length === 0) {
                    ...
                    const itemsWithoutRates = selectedInventoryItems.filter(([_, item]) => !item.rate || item.rate.trim() === '');
    ...
    const renderStepContent = () => {
        switch (currentStep) {
            ...
            case 5:
                ...
                const resetPrimaryGeography = () => {
                    updateFormData({ primaryAudienceGeography: [] });
                };

                const resetSecondaryGeography = () => {
                    updateFormData({ secondaryAudienceGeography: [] });
                };
